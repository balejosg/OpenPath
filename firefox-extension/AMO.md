...
- blocks supported Google Search/Doodles games on managed devices when the
  school policy requires it through background request blocking
...
- Google games enforcement is limited to supported Google Search and
  `doodles.google` request URLs and does not profile general browsing
...
- `<all_urls>`: required to evaluate managed navigation/resource requests and
  apply path, subdomain, and Google game policy across web origins
...
flow, Google games scope, data destination, and local-only native messaging
boundary for Mozilla reviewers.
