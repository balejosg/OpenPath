...
Describe 'PolicyConverter hosted contrast wrapper' {
    BeforeEach {
        ...
        Mock Invoke-OpenPathContrastChild { $script:trace.Add('child'); [pscustomobject]@{ exitCode=1; evidenceAvailable=$true } }
        Mock Write-OpenPathContrastEvidence {}
    }

    It 'enables only the treatment after an observed false baseline and restores false after the child' {
        $result = Invoke-OpenPathPolicyConverterContrast -Mode Enabled -ExecutablePath 'C:\fixture.exe' -ProbePayloadPath 'C:\probe.exe' -ExpectedExecutableSha256 EXE -ExpectedProbePayloadSha256 PROBE -ChildEvidencePath 'C:\child.json' -EvidencePath 'C:\wrapper.json' -TargetUserName 'op-e2e-converter'
        ...
        $result.snapshots.afterIntervention.task.enabled | Should -BeTrue
        $result.snapshots.afterChild.task.enabled | Should -BeTrue
        $result.snapshots.afterRestoration.task.enabled | Should -BeFalse
        $result.restoration.succeeded | Should -BeTrue
        $script:trace | Should -Be @('enable','child','restore')
    ...
    It 'does not coerce an unavailable baseline task to false or invoke the child' {
        ...
        $result = Invoke-OpenPathPolicyConverterContrast -Mode Disabled -ExecutablePath 'C:\fixture.exe' -ProbePayloadPath 'C:\probe.exe' -ExpectedExecutableSha256 EXE -ExpectedProbePayloadSha256 PROBE -ChildEvidencePath 'C:\child.json' -EvidencePath 'C:\wrapper.json' -TargetUserName 'op-e2e-converter'
    ...
    It 'rejects a stopped service whose PID is missing or stale' -ForEach @(
        ...
        $result = Invoke-OpenPathPolicyConverterContrast -Mode Disabled -ExecutablePath 'C:\fixture.exe' -ProbePayloadPath 'C:\probe.exe' -ExpectedExecutableSha256 EXE -ExpectedProbePayloadSha256 PROBE -ChildEvidencePath 'C:\child.json' -EvidencePath 'C:\wrapper.json'
        $result.status | Should -Be 'baseline-not-comparable'
    ...
    It 'restores the treatment after a child invocation failure' {
        Mock Invoke-OpenPathContrastChild { $script:trace.Add('child'); throw 'child-launch-failed' }
        $result = Invoke-OpenPathPolicyConverterContrast -Mode Enabled -ExecutablePath 'C:\fixture.exe' -ProbePayloadPath 'C:\probe.exe' -ExpectedExecutableSha256 EXE -ExpectedProbePayloadSha256 PROBE -ChildEvidencePath 'C:\child.json' -EvidencePath 'C:\wrapper.json'
        $result.status | Should -Be 'child-evidence-unavailable'
        $script:trace | Should -Be @('enable','child','restore')
    }

    It 'restores and writes when the after-child snapshot throws' {
    ...
    It 'marks restoration false when its verification query fails' {
        Mock Get-OpenPathDisposablePolicyConverterObservation {
        ...
        $result.restoration.succeeded | Should -BeFalse
        Should -Invoke Disable-ScheduledTask -Times 1 -Exactly
    }

    It 'stops before intervention when either input hash mismatches' {
        $result = Invoke-OpenPathPolicyConverterContrast -Mode Enabled -ExecutablePath 'C:\fixture.exe' -ProbePayloadPath 'C:\probe.exe' -ExpectedExecutableSha256 WRONG -ExpectedProbePayloadSha256 PROBE -ChildEvidencePath 'C:\child.json' -EvidencePath 'C:\wrapper.json'
        $result.status | Should -Be 'baseline-not-comparable'
    ...
    It 'writes the actual failure-path result with null baseline fields intact' {
        ...
        $null = Invoke-OpenPathPolicyConverterContrast -Mode Disabled -ExecutablePath 'C:\fixture.exe' -ProbePayloadPath 'C:\probe.exe' -ExpectedExecutableSha256 EXE -ExpectedProbePayloadSha256 PROBE -ChildEvidencePath 'C:\child.json' -EvidencePath $path
