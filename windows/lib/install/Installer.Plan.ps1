...
function New-OpenPathInstallPlan {
    ...
    $phases = @(
        ...
        New-OpenPathInstallPhase -Name 'preflight' -Step 0 -TotalSteps 7 -Status 'Ejecutando validacion previa' -Inputs @{ SkipPreflight = $Parameters.SkipPreflight } -RecoveryHint 'Run Pre-Install-Validation.ps1 manually and fix reported prerequisites.'
        New-OpenPathInstallPhase -Name 'directories' -Step 1 -TotalSteps 7 -Status 'Creando estructura de directorios' -RecoveryHint 'Check permissions for C:\OpenPath.'
        New-OpenPathInstallPhase -Name 'runtime' -Step 2 -TotalSteps 7 -Status 'Copiando modulos y scripts' -RecoveryHint 'Verify installer package contents and retry from a local directory.'
        New-OpenPathInstallPhase -Name 'configuration' -Step 3 -TotalSteps 7 -Status 'Creando configuracion' -Inputs $Parameters -RecoveryHint 'Check installer parameters and enrollment configuration.'
