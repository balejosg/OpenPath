...
function toManifestEntry(absolutePath, repoRoot, origin) {
...
function main() {
  ...
  const payloadsDir = args['payloads-dir']
    ...
    : join(repoRoot, 'windows', 'offline-installer', 'build', 'payloads');
  ...
  for (const tree of ['windows', 'runtime']) {
    ...
    for (const filePath of listFilesRecursive(treeRoot)) {
      entries.push(toManifestEntry(filePath, repoRoot, `repo:${tree}`));
    }
  }
