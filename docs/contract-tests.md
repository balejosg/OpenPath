...
| `scripts/verify-full.sh` (e2e gate)                   | `e2e-scope-check.mjs` after all unconditional stages, `OPENPATH_VERIFY_E2E`, `OPENPATH_PREPUSH_REMOTE_SHA`/`OPENPATH_PREPUSH_LOCAL_SHA`, the echo-run crash fallback, `else` branch running `npm run e2e:full` | Renaming the CLI or removing the fallback breaks the test.   |
