...
| `.husky/pre-push`                                     | stdin capture loop `while read -r _local_ref local_sha _remote_ref remote_sha`, single-ref guard `"$ref_count" -eq 1`, `export OPENPATH_PREPUSH_LOCAL_SHA OPENPATH_PREPUSH_REMOTE_SHA`                                                                                                                                                                                                                                                                                                                    | The stdin loop must stay ahead of `npm run verify:full`.                                                                                      |
| `scripts/verify-full.sh` (e2e gate)                   | `node scripts/e2e-scope-check.mjs` after all unconditional stages, exact `OPENPATH_VERIFY_E2E` force-flag comparison, `OPENPATH_PREPUSH_REMOTE_SHA`/`OPENPATH_PREPUSH_LOCAL_SHA`, exact `\|\| echo run` crash-fallback expression, exact `== "skip"` scope-decision comparison, and an end-of-file anchor pinning the outer `else` / `npm run e2e:full` / `fi` as the script's FINAL branch (not just any `else...fi` substring, which would also match the inner not-skip branch nested inside the elif) | Renaming the CLI, removing the fallback, or changing the skip comparison breaks the test; keep the outer else as the last branch in the file. |
