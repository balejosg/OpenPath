...
@test "add_extension_to_policies purges a stale managed extension id sharing the install_url" {
    source "$PROJECT_DIR/linux/lib/browser.sh"

    # Simulate a policy generated by an older agent that force-installed the
    # same managed XPI under a legacy extension id.
    run add_extension_to_policies \
...
ext_settings = policy_root.get("ExtensionSettings", {})
assert "monitor-bloqueos@openpath" not in ext_settings, ext_settings
assert ext_settings["openpath-block-monitor@openpath"]["installation_mode"] == "force_installed", ext_settings
locked = policy_root.get("Extensions", {}).get("Locked", [])
assert "monitor-bloqueos@openpath" not in locked, locked
assert "openpath-block-monitor@openpath" in locked, locked
PYEOF
}

@test "add_extension_to_policies preserves unrelated admin-managed extensions" {
    cat > "$FIREFOX_POLICIES" <<'JSON'
{"policies": {"ExtensionSettings": {"admin-tool@example": {"installation_mode": "force_installed", "install_url": "https://addons.example/admin-tool.xpi"}}}}
JSON
...
ext_settings = policies["policies"]["ExtensionSettings"]
assert ext_settings["admin-tool@example"]["install_url"] == "https://addons.example/admin-tool.xpi", ext_settings
assert "openpath-block-monitor@openpath" in ext_settings, ext_settings
PYEOF
}
