...
test('real NSIS failures expose only a bounded installer phase for diagnosis', () => {
  ...
  assert.match(
    installer,
    /function Set-OpenPathOfflinePayloadFailurePhase[\s\S]*hash-io\|size-io[\s\S]*\$Matches\[1\][\s\S]*offline-payload-verification-\$suffix/,
  ...
  assert.match(
    offlineModule,
    /ValidateSet\('manifest', 'entry', 'missing', 'sha256', 'size', 'hash-io', 'size-io', 'io', 'failed'\)[\s\S]*Offline payload verification failed \[\$Category\]/,
