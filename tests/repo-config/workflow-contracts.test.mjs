...
test('WEDU captive portal lab workflow is manual or nightly and restores the shared runner', () => {
  ...
  assert.ok(
    script.includes('github_api_get "repos/${repository}/actions/runners"') &&
    ...
    'WEDU lab script should query runner state without requiring the GitHub CLI or repository-runner API access on the controller'
  );
  assert.ok(
...
test('required Windows CI runs Pester in an untracked child host without success shortcuts', () => {
  ...
  assert.ok(
    windowsHostedJobBlock.includes('timeout-minutes: 20'),
    'ci.yml should keep the hosted Windows gate bounded so genuine stalls fail'
  );
  assert.ok(
  ...
  assert.ok(
    windowsHostedJobBlock.includes('tests/e2e/ci/run-windows-pester-isolated.ps1') &&
      windowsHostedJobBlock.includes(
        '-ResultsPath windows-hosted-results-${{ matrix.shard }}.xml'
      ) &&
