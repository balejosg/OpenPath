...
export function getStudentPolicyPhasePlan(
  ...
  if (coverageProfile === 'dns-discovery-spike') {
    if (mode !== 'sse') {
      throw new Error('The dns-discovery-spike coverage profile requires sse mode');
    }

    return [{ name: 'dns-discovery-spike', suite: 'dns-discovery-spike', useBrowser: true }];
  }

  if (coverageProfile === 'dns-evidence-matrix') {
    if (mode !== 'sse') {
      throw new Error('The dns-evidence-matrix coverage profile requires sse mode');
    }
    ...
    for (const phase of getStudentPolicyPhasePlan(mode, coverageProfile, scenarioGroup)) {
      await runPhase(
        ...
        async (driver) => {
          ...
          if (phase.suite === 'dns-discovery-spike') {
            await runDnsDiscoverySpikeScenario(client, driver, mode);
            return;
          }
